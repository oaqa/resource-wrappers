...
public class EntityTermConverter {
	...
	public static Term EntityToTerm(Entity ent) {
		...
		// Add type
		term.addTermRelationship(new TermRelationship(term.getTerm(), "type", ent.getType(), 1.0, ent.getSource()));
		// Add definition
		term.addTermRelationship(new TermRelationship(term.getTerm(), "definition", ent.getDefinition(), 1.0, ent.getSource()));
		// Add ID's
		for (ID id : ent.getIDs())
			term.addTermRelationship(new TermRelationship(term.getTerm(), "ID", id.toString(), 1.0, ent.getSource()));
		// Add synonyms
		for (String syn : ent.getSynonyms())
			term.addTermRelationship(new TermRelationship(term.getTerm(), "synonym", syn, 1.0, ent.getSource()));
		// Add other relationships
		for (Relation rel : ent.getRelations())
			term.addTermRelationship(new TermRelationship(term.getTerm(), rel.getProperty(), rel.getValue(), 1.0, ent.getSource()));
